...
import Services.ImageDetailService;
...
//                message = status ? "Chỉnh sửa màu thành công" : "Chỉnh sửa màu thất bại vui lòng kiểm tra lại";
			String codeProduct = request.getParameter("id-Product");
			String oldPath = request.getParameter("old-path");
			String color = request.getParameter("color-product");
			Part part = request.getPart("image");
			String readPath = request.getServletContext().getRealPath("/assets/imgs/product-imgs");
			message = ImageDetailService.updateImageDetail(codeProduct, color, part, oldPath, readPath);
			break;
		}
		default:
			break;
